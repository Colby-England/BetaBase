...
app.put('/editGym/:id', (req, res) => {
    db.pool.query(getGym,[req.params.id], function(err, result){
        if(err){
            next(err);
        return;
        }
        ...
        if(result.length == 1)
        {
            ...
            db.pool.query("UPDATE gyms SET name=?, city=?, stateID=?, address=?, cost=? WHERE id=? ",
            [req.body.name || curVals.name, req.body.city|| curVals.city, req.body.stateID || curVals.stateID, 
                req.body.address || curVals.address, req.body.cost || curVals.cost, req.params.id],
            function(err, result)
            {
                if(err)
                {
